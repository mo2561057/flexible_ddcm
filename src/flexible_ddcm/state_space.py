...
def _create_state_space_array(states, model_options):
    ...
    # Reset index
    state_space.index = range(state_space.shape[0])
    # Build covariats
...
def create_derived_opjects(state_space, choice_key_to_choice_set, state_choice_space):
    ...
    for key in state_and_choice_to_state_choice:
        state_and_choice_to_state_choice[key][
            state_choice_space[state_choice_space.choice==key].state_key.values
        ] = state_choice_space[state_choice_space.choice==key].index.values.astype(int)
