...
def process_simulation_dict(
    ...
    long_df = _create_long_df(simulate_dict, params, wage_periods, additional_cols)
    wide_df = _create_wide_df(simulate_dict, schooling_levels)
    return {"wide": wide_df, "long": long_df}


def _create_long_df(simulate_dict, params, wage_periods, additional_cols):
    ...
    for period in wage_periods:
        # Check all people
        current_df = terminal_df[terminal_df.period <= period]
    ...
    out = out.join(
        simulate_dict[max(simulate_dict.keys())][additional_cols]
    ).reorder_levels(["Identifier", "period"])
    return out
...
def _create_wide_df(simulate_dict, schooling_levels):
    ...
    out["degree_combination"] = grouper.ngroup().replace(rename_dict)

    # Get ever enrolled:
    choice_df = pd.concat([df["choice"] for df in simulate_dict.values()], axis=0)
    for level in schooling_levels:
